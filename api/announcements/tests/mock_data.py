...
__MULTI_EVENT_PAYLOAD = {
    k: v for k, v in ANNOUNCEMENT_PAYLOAD.items() if k != "event_time"
}
__SINGLE_EVENT_PAYLOAD = {
    k: v if v != "multi_event" else "single_event"
...
POST_SINGLE_EVENT_NO_MESSAGE_IN_PAYLOAD_201 = (
    {
        k: v
        for k, v in __SINGLE_EVENT_PAYLOAD.items()
        if k not in ["message", "event_end_date"]
    },
    201,
    __success_response,
)
...
POST_SINGLE_EVENT_NO_EVENT_START_DATE_IN_PAYLOAD_403 = (
    {
        k: v
        for k, v in __SINGLE_EVENT_PAYLOAD.items()
        if k not in ["event_start_date", "event_end_date"]
    },
    403,
    {"error": "event_start_date is required"},
)
...
POST_SINGLE_EVENT_NO_EVENT_TIME_IN_PAYLOAD_403 = (
    {
        k: v
        for k, v in __SINGLE_EVENT_PAYLOAD.items()
        if k not in ["event_end_date", "event_time"]
    },
    403,
...
POST_SINGLE_EVENT_NO_TITLE_IN_PAYLOAD_403 = (
    {
        k: v
        for k, v in __SINGLE_EVENT_PAYLOAD.items()
        if k not in ["event_end_date", "title"]
    },
    403,
    {"error": "title is required"},
)
